...
public class SausSpel implements Serializable {
	...
	public SausSpel(SausDAO sausDAO) {
		...
		this.saus = sauzen.get(new Random().nextInt(sauzen.size())).getNaam(); // mooie
																				// constructie
																				// :-)
																				// int
																				// i
																				...
																				// new
																				// Random().nextInt(new
																				// ArrayList<Saus>((ArrayList)new
																				// SausDAO().findAll()).size());
	...
	public void raden(String gok) {
		...
		if (this.juist.equals(j.toString()) && saus.indexOf(gok.charAt(0)) == -1) {
			this.keerGeraden++;
			if (this.keerGeraden == MAX_BEURTEN) {
				this.spelstatus = "verloren";
			}
		} else {
			this.juist = j.toString();
			if (j.indexOf(".") == -1) {
				this.spelstatus = "gewonnen";
			}
		}
	}
